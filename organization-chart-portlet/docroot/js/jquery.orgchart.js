...
(function(factory) {
  ...
  function createNode(nodeData, level, opts) {
    var dtd = $.Deferred();
	var $parentClass = $('.nodes td').first().find('.node').attr("class");
    // construct the content of node
	//var $tooltip = ('<div class="tooltip">'+nodeData[opts.nodeTitle]+'</div>');
    var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.nodeId] ? ' id="' + nodeData[opts.nodeId] + '"' : '') + 'data-toggle="tooltip" data-placement="top" title="'+nodeData.tooltip+'"  treeLevel="'+nodeData.treeLevel+'" nodeLevel="'+nodeData.nodeLevel+'" >')
	...
	//$nodeDiv.prepend($tooltip);

	//$('.nodes td').first().find('.node').removeClass().split(' ')[1];
    ...
    if (opts.draggable) {
      ...
      .on('drop', function(event) {
		...
		var $parentClassName = ($dropZone).attr("class").split(' ')[1];
